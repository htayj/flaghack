...
3. **Introduce explicit game/session/state ownership.** Replace the global singleton with a `GameSession` / `GameStateStore` abstraction keyed by game/player/session.
...
### 12. State ownership has no session/persistence boundary

- **Evidence:** global mutable singleton initialized in `packages/server/src/gameloop.ts:47-72`, mutated at `:74-76`; `GameRepository` forwards to imported module state at `packages/server/src/GameRepository.ts:15-30`; inventory is hard-coded to `"player"` at `GameRepository.ts:29`; `getPlayer` hard-codes `"player"` at `packages/server/src/gamestate.ts:19-22`.
- **Current architecture smell:** One process-global game exists. Repository/session/persistence boundaries are absent.
...
### 18. Mutation endpoint returns no authoritative post-action state

- **Evidence:** `doAction` declares payload but no success body in `packages/domain/src/GameApi.ts:24-27`; clients manually do action -> getWorld and sometimes separate inventory reads at `packages/web/src/Playing.tsx:127-136` and `packages/cli/src/components/BPlaying.tsx:140-152`; pickup/drop often do not refresh state at `packages/web/src/Playing.tsx:188-194`, `packages/cli/src/components/BPlaying.tsx:158-176`.
- **Current architecture smell:** Mutations are fire-and-requery; clients decide what is fresh, and each platform does it differently.
...
### 20. State freshness and atomicity are not modeled

- **Evidence:** `World` / `GameState` have no turn/version field at `packages/domain/src/schemas.ts:299-300`; server action workflow reads, computes, then sets state separately in `packages/server/src/gameloop.ts:78-85`; mutable state-changing data is exposed by GET endpoints in `GameApi.ts:7-22`.
- **Current architecture smell:** Clients cannot detect stale snapshots; concurrent actions have no explicit sequencing.
