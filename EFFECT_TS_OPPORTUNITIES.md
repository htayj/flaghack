...
2. **Move server state/logs into Effect services.** Replace module-level mutable state with `Ref`/`SynchronizedRef`/`Queue`-backed services provided by `Layer`s.
...
| **Partial**   | 3-5, 7-8, 17-24: server state, game-loop effects, logging, AI/pathfinding/config/collection boundaries              | Pure action reducer work, UUID keys, log snapshots/caps, immutable pathfinding distance handling, safer indexing, creature-only synchronous AI planning, and lazy state initialization reduce risk.                                                           | Module-level `_state` remains; no `GameStateStore`, `Ref`/`SynchronizedRef`, atomic update, session/persistence, turn/version model, runtime config layer, or complete collection policy is claimed.                                                                        | Targeted reducer/log/pathfinding/world tests plus smoke gates.                                          |
...
| **Partial**   | 9-16, 32-35, 40-41, 50-51: CLI/web runtime, UI effects, duplicate client/UI utilities, metadata, and error handling | App-scoped runtimes/layers, lifecycle-based initial fetches, key handler cleanup, ignored unknown inputs, post-action refresh, popup selection/focus improvements, capped messages, browser CORS leakage removal, and baseline web accessibility reduce risk. | Shared client/UI-model extraction, complete remote-data/UI state machine, supervised UI runtime/error handling, timeout/retry/user-visible error handling, and final CLI metadata polish are deferred.                                                                      | CLI/web unit smoke and API/E2E smoke gates as applicable.                                               |
...
### 3. Server game state is a mutable module singleton

- **Evidence:** `packages/server/src/gameloop.ts:47-72`, `:74-89`, `:129-131`; `packages/server/src/GameRepository.ts:12-39`.
- **Current pattern:** `_state` is a module-level mutable object hidden behind effects.
...
### 4. Game-state updates are not atomic

- **Evidence:** `packages/server/src/gameloop.ts:78-89`, `:95-127`.
- **Current pattern:** Workflows do `get state -> compute -> set state` as separate effects.
