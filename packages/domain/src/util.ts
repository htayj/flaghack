...
type AllOf<T extends [S.Schema.Any, ...Array<S.Schema.Any>]> = T extends [
  infer First extends S.Schema.Any,
  infer Second extends S.Schema.Any,
  ...infer Rest extends Array<S.Schema.Any>
] ? AllOf<[S.extend<First, Second>, ...Rest]>
  : T[0] // when only one left, return it

export function allof<T extends [S.Schema.Any, ...Array<S.Schema.Any>]>(
  ...schemas: T
): AllOf<T> {
...
export const collect = <T extends [S.Schema.Any, ...Array<S.Schema.Any>]>(
  ...items: T
) =>
  [
    allof(...items),
